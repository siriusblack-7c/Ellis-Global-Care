...
import { Heart, Shield, Users, Award, Clock, MapPin, Coffee, Home } from "lucide-react";
...
export default function Amenities() {
  ...
  const getIcon = (categoryName: string, index: number) => {
    const icons = {
      care: [<Heart key={0} />, <Shield key={1} />, <Users key={2} />, <Award key={3} />],
      support: [<Clock key={0} />, <Coffee key={1} />, <Home key={2} />, <MapPin key={3} />],
      training: [<Award key={0} />, <Shield key={1} />, <Users key={2} />, <Heart key={3} />],
      locations: [<MapPin key={0} />, <Users key={1} />, <Home key={2} />, <Clock key={3} />]
    };
    
    return icons[categoryName as keyof typeof icons]?.[index] || <Heart />;
  };
